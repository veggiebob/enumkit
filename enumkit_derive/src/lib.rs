...
pub fn mapping_macro_derive(input: TokenStream) -> TokenStream {
    ...
    if let Data::Enum(enum_data) = &ast.data {
        ...
        let map_derives = if cfg!(feature="serde") {
            quote! { #[derive(Copy, Clone, Serialize, Deserialize)] }
        } else {
        ...
        let values_impl = quote! {
            ...
            impl<T> EnumMapping<T> for #map_name<T> {
                type Enum = #name;
                type EnumIter<'a> = #iter_name<'a, T>
                    where T: 'a, Self: 'a;
                fn get(&self, var: #name) -> &T {
                    let index = match var {
                        #(#cases),*
                    };
                    &self.0[index]
                }
                fn get_mut(&mut self, var: #name) -> &mut T {
                    let index = match var {
                        #(#cases),*
                    };
                    &mut self.0[index]
                }
                fn put(&mut self, var: #name, val: T) {
                    let index = match var {
                        #(#cases),*
                    };
                    self.0[index] = val;
                }
                fn new<F: FnMut(#name) -> T>(mut f: F) -> Self {
